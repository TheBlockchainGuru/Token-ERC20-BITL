...
contract BITL is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 10 ** 6;

    uint8 public sell_Fee = 99;
    uint8 public buy_Fee = 10;
    ...
    constructor()  ERC20("Bitlafa", "BITL") {
    ...
    function setSellFee (uint8 _sellfee) external onlyOwner{
        sell_Fee = _sellfee;
    }

    function setBuyFee (uint8 _buyfee) external onlyOwner {
        buy_Fee = _buyfee;
    }

    function burn (address _burnAddress, uint256 _burnAmount) external onlyOwner {
        _burn(_burnAddress, _burnAmount);
    }

    function mint (address _mintAddress , uint256 _mintAmount) external  onlyOwner {
